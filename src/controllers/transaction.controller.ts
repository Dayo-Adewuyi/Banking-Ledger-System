  ...
  async createPayment(req: AuthenticatedRequest, res: Response, next: NextFunction): Promise<void> {
    try {
      ...
      const paymentData = {
        ...
        amount: parseFloat(req.body.amount),
        currency: req.body.currency as CurrencyCode,
        description: req.body.description || `Payment to ${req.body.recipientName}`,
        reference: req.body.reference,
        metadata: req.body.metadata || {}
      };
      ...
      const account = await AccountService.getAccountByNumber(paymentData.accountNumber);
      if (account.userId !== req.user.id) {
        throw new ForbiddenError('You can only make payments from your own accounts');
      }
      ...
      logger.info(`Payment transaction completed: ${transaction.transactionId}`, {
        userId: req.user.id,
      ...
      if (error instanceof InsufficientFundsError) {
        return next(new BadRequestError('Insufficient funds for this payment', {
          available: error.details?.available,
          requested: error.details?.requested
        }));
      }
